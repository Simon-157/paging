...
char accessMemory(int physical_address, int write) {
    ...
    if (write) {
        ...
        page_table[physical_memory[frame].page_number].dirty = 1;
...
int loadPageIntoMemory(int page, int process_id) {
    ...
    // Update the page table
    page_table[frame].frame_number = frame;
    page_table[frame].valid = 1;
    page_table[frame].dirty = 0;
    page_table[frame].lru_counter = lru_counter++;
